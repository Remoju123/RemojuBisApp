...
export class PlanSpotListService {
  ...
  getGoogleSpotList(guid: string, keyword: string, token: string) {
    ...
    return this.http.post<GoogleSearchResult>(
      url,
      {
        keyword: keyword,
  ...
  fetchDetails(planSpotList: PlanSpotList, guid: string) {
    if (planSpotList.isPlan !== undefined && planSpotList.isPlan) {
      ...
      return this.http.get<PlanSpotList>(
        this.host + '/api/PlanSpotList/SearchDetailSpot',
        {
          params: {
            versionNo: planSpotList.versionNo,
  ...
  mergeDetail(
    ...
    return new Promise((resolve) => {
      try {
        ...
        detail.userName = this.commonService.isValidJson(
          detail.userName,
  ...
  getFilterbyCondition(
    ...
    return new Promise((resolve) => {
      ...
      _result = _result.filter((item: { isRemojuPlan: boolean }) => {
        ...
        return item.isRemojuPlan
          ? item.isRemojuPlan === cond.isRemojuRecommended
      ...
      // キーワード検索
      if (cond.keyword !== '' && cond.keyword !== undefined) {
        const keywords = cond.keyword.replace('　', ' ').split(' ');
        if (keywords.length > 0) {
          _result = _result.filter((d) =>
            keywords.every(
              (x) => d.keyword && d.keyword.toLowerCase().indexOf(x) !== -1
            )
          );
      ...
      if (!master) {
        ...
        * 2.検索条件文字列結合
        ...
        if (master.mSearchCategoryPlan) {
          const $master = master.mSearchCategoryPlan.concat(
            master.mSearchCategory
          );
        ...
        * 3.検索パラメータ結合
        ...
        this.result.list = _result;
        this.result.searchTarm = {
          area: areas.length > 0 ? areas.join(' 、') : '----',
          cate: cates.length > 0 ? cates.join('、') : '----',
        };
  ...
  dataFormat(row: PlanSpotList) {
    row.planName = this.commonService.isValidJson(
      row.planName,
      this.translate.currentLang
    );
  }
  ...
  reduceMasterCategory(
    ...
    return mSearchCategory.reduce((x, c) => {
      x.push({
        ...
        parentName: c['parentName'],
        qty: list.filter((x) =>
          x.searchCategoryIds !== null
            ? x.searchCategoryIds.some(
                (s: { toString: () => string }) =>
                  s.toString().slice(0, 3) === c['parentId'].toString()
              )
            : []
        ).length,
        selected: true,
        dataSelecteds: c['dataSelecteds'].reduce((y, d) => {
          y.push({
            ...
            qty: list.filter((x) =>
              x.searchCategoryIds !== null
                ? x.searchCategoryIds.some((s: number) => s === d['id'])
                : []
            ).length,
