...
export class DataService {
  ...
  async delete(index: number) {
    ...
    if (tmp.isPush) {
      if (tmp.alarmTimestamp - Date.now() < 30) {
        alert("アラーム時刻まで30秒以内は削除してもPush通知が届きます");
        return;
      }
  ...
  // Service Workerの状況確認
  @Catch()
  private async isReadySW(): Promise<{ success: boolean; data: any }> {
  ...
  private async resetSW() {
    const ret = await this.isReadySW(); // Service Worker
    if (ret.success) {
      // 登録済みのService Workerを1件づつ登録解除
      for (const registration of ret.data.regs) {
        const scope = registration.scope;
        const r = await registration.unregister();
        console.log("Service Worker登録解除" + scope + "," + r);
      }
      // ngsw:を名前に含むものを削除
      for (const cacheName of ret.data.swCaheNames) {
        const r = await caches.delete(cacheName);
        console.log("削除したキャッシュ" + cacheName + "," + r);
      }
    } else {
      throw new Error("ServiceWorkerが利用できません");
    }
