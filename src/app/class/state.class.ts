...
export class State {
  ...
  onRestore() {
    ...
    if (json) {
      ...
      // オブジェクト１つにばらして上書きコピー
      if (obj.alarmList > 0) {
        this.alarmList = obj.alarmList.map((alarmData: Partial<Alarm> | undefined) =>
          Object.assign(new Alarm(alarmData))
        );
      }
      // 前回終了時のテーマを復元
  ...
  // ローカルアラームの登録
  @Catch()
  addLocalAlarm(alarm: Alarm, timeup: { (alarm: Alarm): void; (arg0: Alarm): void; }) {
    // アラーム一覧登録とタイマー開始
  ...
  // 状態保存
  @Catch()
  private save(state: { alarmList: Alarm[]; theme: string; isEnablePush: boolean; }) {
    let json;
    if (state) {
  ...
  private sortAlarmList() {
    if (this.alarmList.length > 1) {
      this.alarmList.sort((a, b) => a.alarmTimestamp - b.alarmTimestamp);
    }
  }
}
