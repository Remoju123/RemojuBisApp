...
export class Alarm {
  ...
  timer: any; // number?         // setTimeoutの戻り値
  ...
  // タイマーの設定
  @Catch()
  startTimer(timeup: (arg0: this) => void) {
    // 残り時間を取得
    const rest = this.alarmTimestamp - Date.now();
    // タイマー起動
  ...
  @Catch()
  update(): Alarm {
    this.alarmTimestamp = Date.now() + this.timerValue * 60 * 1000;
    return this;
  }

  // アラーム設定時刻の文字表現取得
  @Catch()
  getTimeString(): string {    const d = new Date(this.alarmTimestamp);
    const h = d.getHours();
  ...
  updateAlarmNameCounter(): string {
    //const ret = localStorage.getItem("count");
    const ret = 0;
