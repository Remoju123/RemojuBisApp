...
export class UserPlanListComponent implements OnInit {
          ...
          this.userService
            ...
            .subscribe((r) => {
              ...
              this.searchCategories =
                this.planSpotListService.getMasterCategoryNames(
                  new Set(ids),
                  r.mSearchCategoryPlan
                );
  ...
  mergeNextDataSet() {
    if (this.userPlanSpots.length > 0) {
      ...
      for (let i = startIndex; i < this.end; i++) {
        ...
        this.planspots
          .fetchDetails(this.userPlanSpots[i], this.guid)
          .pipe(takeUntil(this.onDestroy$))
          .subscribe(async (d) => {
            this.userPlanSpots[i] = await this.planspots.mergeDetail(
              this.userPlanSpots[i],
              d
            );
            this.details$ = this.userPlanSpots.slice(0, this.end);
          });
  ...
  linktoDetail(item: PlanSpotList) {
    if (item.isPlan) {
      this.router
        .navigate(['/' + this.lang + '/plans/detail', item.id])
        .then(() => {});
    } else {
